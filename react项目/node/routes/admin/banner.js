...
router.post('/', (req, res, next) => {
  ...
  let { name, brandname,newprice,oldprice } = req.body;//拆除body数据
  ...
  mgdb(
      ...
      ({ collection, client }) => {
        collection.insertOne(
            { name, brandname,newprice,oldprice, banner, time, detail: {icon} }
            ,
            (err, result) => {
              if (!err && result.result.ok) {
                // res.send({ error: 0, mess: '成功', data:  result.result.ops[0] })
                res.send({ error: 0, mess: '成功', data: { id: result.insertedId, name, brandname,newprice,oldprice, banner, time, detail: { icon} } })


                //广播给所有客户端
                let io=require('../../bin/www');
                io.emit('mall_banner', {data: { id: result.insertedId, name, brandname,newprice,oldprice, banner, time, detail: { icon}}})
...
router.patch('/:id', (req, res, next) => {
  ...
  mgdb({
    ...
    collection.find({
      ...
      //传过来的数据 和 库数据  合并
      let {name, brandname,newprice,oldprice} = req.body;
      name = name || result[0].name;
      brandname = brandname || result[0].brandname;
      newprice = newprice || result[0].newprice;
      oldprice = oldprice|| result[0].oldprice;
